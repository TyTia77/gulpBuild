...
JobList = Polymer({
	...
	observers: ["_handleFieldSplices(fields.splices)", "_onJobsChanged(jobs.*, _activeFilter)"],
	...
	notifyJobUpdated: function notifyJobUpdated(updatedJob, jobBeforeUpdate) {
	...
	_onRowMouseover: function _onRowMouseover(ev) {
		ev.target.draggable = true;
	},
	_onRowMouseout: function _onRowMouseout(ev) {
		ev.target.draggable = false;
	},
	_onRowDragstart: function _onRowDragstart(ev) {
	...
	_getFieldValue: function _getFieldValue(obj, field) {
		var fieldKey = field["key"];
		return obj[fieldKey];
	},
	_getClassForIndex: function _getClassForIndex(index) {
		return index % 2 == 0 ? "even-row" : "odd-row";
	},
	_disableColumnFlex: function _disableColumnFlex() {
	...
	_handleResizeDrag: function _handleResizeDrag(e) {
	...
	_handleRowMouseDown: function _handleRowMouseDown(ev) {
		var longPressTimer;
	...
	_onIronListScroll: function _onIronListScroll(ev) {
	...
	_getArrowImageForOpenedState: function _getArrowImageForOpenedState(opened) {
		return this.resolveUrl("./assets/images/greyArrow_" + (opened ? "down" : "up") + ".png");
	},
	_handleCollapseTapped: function _handleCollapseTapped() {
		this.opened = !this.opened;
	},
	_getClassForFixedColumnState: function _getClassForFixedColumnState(fixedColumns) {
		return fixedColumns ? "fixed" : "";
	},
	_getColumnWidth: function _getColumnWidth(fixedColumns, fixedWidth) {
		return fixedColumns ? fixedWidth : "auto";
	},
	_injectColumnWidth: function _injectColumnWidth(index, columnWidth) {
	...
	_refreshColumnStyles: function _refreshColumnStyles() {
	...
	_handleFieldsChanged: function _handleFieldsChanged(fields) {
		this._refreshColumnStyles();
	},
	_handleFieldSplices: function _handleFieldSplices(fieldSplices) {
		if (fieldSplices !== undefined) {
			this._refreshColumnStyles();
		}
	},

	_onJobsChanged: function _onJobsChanged(jobChangeSet, activeFilter) {
		...
		if (jobChangeSet.path === "jobs") {
			...
			for (var i = 0, len = jobChangeSet.value.indexSplices.length; i < len; i++) {
				...
				if (indexSplice.addedCount > 0) {
					var addedJobs = indexSplice.object.slice(indexSplice.index, indexSplice.index + indexSplice.addedCount);
	...
	_processJobAddedSet: function _processJobAddedSet(jobs, activeFilter) {
	...
	_processJobChanged: function _processJobChanged(job, activeFilter) {
	...
	_processJobDeletionSet: function _processJobDeletionSet(jobs) {
	...
	_getFilteredJobIndex: function _getFilteredJobIndex(rcJobId) {
	...
	_getColumnClassForIndex: function _getColumnClassForIndex(index) {
	...
	_handleResizeHandleCreated: function _handleResizeHandleCreated(ev) {
	...
	_getLargestCellWidth: function _getLargestCellWidth(fieldName) {
		...
		var cellWidth = largestTextWidth + (paddingLeft + paddingRight) + (marginLeft + marginRight) + (borderLeft + borderRight);

		return cellWidth;
	},

	_getLargestTextWidthForField: function _getLargestTextWidthForField(fieldName) {
